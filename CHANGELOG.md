...
在之前的版本中，SadeRift 采用了 **MIT** 开源协议。然而，我们发现部分开发者对软件进行直接套壳，并将其用于商业用途，虽然这仍然符合 MIT 协议，但与 SadeRift 的开发初衷相悖。

v1.3.0 版本对底层代码进行了重构，同时，自本版本起，SadeRift 的开源协议变更为 **GPL-3.0**。
...
- SadeRift 现在会尝试执行自动更新，通过 Gitee 或 Github 仓库。
...
- 现在 SadeRift 将会显示一个托盘区图标，用于快速最小化以及唤起副窗口。

- 提供了一个副窗口，其可指示一些正在进行的操作。托盘区 -> `SadeRift` -> `显示小窗`。
...
- 现在 SadeRift 会记录日志，日志将写入到程序目录下的 `logs/LeagueAkari_{datetime}.log`。可以通过 `设置` -> `调试` -> `日志` -> `日志目录` 来查看日志文件。
...
在一些游戏状态（英雄选择、房间内）时，SadeRift 会开启一个副窗口，用于指示即将进行的自动化操作，并提供一些快捷操作入口。
...
- 现在数据库添加了额外的字段，以记录 SadeRift 的使用者以及所属大区，避免潜在的混淆。
...
- 提供一个方式，可以断开 SadeRift 与 LCU 的连接。位于 `设置` -> `进程` -> `League Client` -> `断开连接`。
